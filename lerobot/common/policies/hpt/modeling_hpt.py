...
class HPTPolicy(
    ...
    def reset(self):
        ...
        self.history_buffer = defaultdict(deque, maxlen=self.model.n_obs_steps)
...
class HPT(nn.Module):
    ...
    def forward_features(self, data: dict) -> torch.Tensor:
        ...
        return torch.cat([data["observation.state"], data["observation.environment_state"]], dim=2).flatten(
            start_dim=1
        )
