...
class HPTPolicy(
    ...
    def __init__(
        ...
        config.n_action_steps = config.action_horizon
    ...
    #         while len(self.history_buffer[modality]) < self.model.observation_horizon:
    ...
    def init_domain_head(self, domain_name):
        ...
        if self.config.head_architecture == "diffusion":
            self.heads[domain_name] = DiffusionHead(
                config=self.config,
                embed_dim=self.config.state_input_dim * self.config.observation_horizon,  # head_input_dim,
